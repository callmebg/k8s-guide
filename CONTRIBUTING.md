...
每次新增、删除或重新编号文章时，**必须同步更新以下三处**：

| # | 位置 | 操作 |
|---|------|------|
| 1 | `README.md` 进度表 | 更新对应文章的状态（⬜ → ✅） |
| 2 | `docs/.vitepress/config.ts` 侧边栏 | 添加/删除/调整条目顺序 |
| 3 | 前后文章的 frontmatter | 更新 `prev` / `next` 链接 |

**PR 自检**：提交 PR 前请确认以上三处已全部更新。
